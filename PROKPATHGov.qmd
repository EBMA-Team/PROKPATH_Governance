...
A static registry snapshot was retrieved and formatted based on the fixed date of preparation of the snapshot (31-Jul-2025).
...
gg_miss_fct(PROMKA1 |> dplyr::select(CreationYear,KOOSJRTotalScore), CreationYear) + scale_fill_viridis_c(
...
gg_miss_fct(PROMTFI1, CreationYear) + scale_fill_viridis_c(
    option = "plasma",  # or "inferno", "magma", "cividis"
    name = "% Missing",
    limits = c(30, 90)  # Set limits from 0% to 100%
  )

```

The results displayed in @fig-ka-missbase and @fig-tfi-missbase show that there are remaining issues with the capture of baseline data for key cohorts.

We have identified modifications to the cohort identification process that may be increasing the number of patients allocated to the General cohort, that are subsequently moved to the correct cohort after the initial consultation. While this is conceptually correct and minimises the risk of presentations being allocated to the wrong cohort initially, it seems to have created a barrier in retrieving cohort-specific PROMs. It may be that the PROMs pack for the General cohort needs to be amended to reduce the burden (it currently contains the full KOOS), make assumptions about cohort allocation prior to the initial consultation, or hold off sending any PROMs until the initial consultation has occurred.
...
-   The results flag issues with the capture of baseline PROMs in the registry

    -   Cohort-specific PROMs are being ignored by patients at baseline

-   Intraoperative data is lacking for key cohorts

-   The registry identifies unique patterns in key patient-reported outcomes such as satisfaction, instability symptoms, pain and function overall.

# Recommendations

-   Amend the enrolment and PROMs capture process at baseline

-   EBMA has developed a comprehensive form for intraoperative capture which is ready to pilot

-   Plan for analyses of the existing data in key cohorts and patient subgroups of interest.
